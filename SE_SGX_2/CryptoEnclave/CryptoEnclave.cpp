...
void ecall_init(unsigned char *keyF, size_t len){ 
    ...
    // uint64_t vector_size = 37209489;//4mb hold up to 1.5 million key,value pairs
    uint64_t vector_size = 360000;//4mb hold up to 1.5 million key,value pairs
    uint8_t numHashs = 23;
...
void ecall_printMem(){
    printf("%d", (sizeof(std::string)+sizeof(int))*ST.size()+(sizeof(std::string))*d.size());
}
