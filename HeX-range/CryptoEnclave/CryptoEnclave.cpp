...
void ecall_updateDoc(char *doc_id, size_t id_length, char *content, size_t content_length, const int* op, size_t op_len){             
    ...
    // printf("%d \n", CK.size());
    unsigned long num = (unsigned long) atoi(wordList[0].c_str());
    std::bitset<bits> num_bin = std::bitset<bits>(num);
    std::string num_bin_str = num_bin.to_string();

    std::vector<string> num_wordList;
    for (int i=0; i < bits; i++) {
        std::string w;
        if (i==0) {
            if (num_bin[bits-1-i]==0) {
                w = std::to_string(1) + ">";
            }else {
                w = std::to_string(0) + "<";
            }
            ...
            if (num_bin[bits-1-i]==0) {
                w = num_bin_str.substr(0,i) + std::to_string(1) + ">";
            }else {
                w = num_bin_str.substr(0,i) + std::to_string(0) + "<";
            }
        }
        num_wordList.push_back(w);
    }
    std::shuffle(num_wordList.begin(), num_wordList.end(), std::mt19937{std::random_device{}()});


    for(std::vector<string>::iterator it = num_wordList.begin(); it != num_wordList.end(); ++it) {     
...
void ecall_search(const int *keyword, size_t keyword_len, const char *order_char, size_t order_len){
    //init keys
    unsigned int num = (unsigned int) *keyword;
    string order(order_char, order_len);

    std::string num_bin_str = std::bitset<bits>(num).to_string();
    // std::reverse(num_bin_str.begin(), num_bin_str.end());

    std::vector<string> num_wordList;
    for (int i=0; i < bits; i++) {
        std::string w;
        if (i==0) {
            w = num_bin_str[i] + order;
        }else {
            w = num_bin_str.substr(0,i) + num_bin_str[i] + order;
            
        }
        num_wordList.push_back(w);
    }
    std::shuffle(num_wordList.begin(), num_wordList.end(), std::mt19937{std::random_device{}()});
    std::bitset<id_max> bm_result;
    ...
    // printf("total id number: %d \n", bm_result.count());
    bm_result.count();
}

void ecall_printMem(){
    printf("%d", (sizeof(string)+sizeof(int))*KVS.size()+(sizeof(string)+10000/8)*CK.size());
}
