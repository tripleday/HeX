...
class App_Controller:
    ...
    def training_data_parser(self, origin_dir="maildir", process_dir = "enron"):
        ...
        for dirName, subdirList, fileList in os.walk(dst_dir):
            ...
            for email_doc in fileList:
                ...
                fp = open(os.path.join(dirName, email_doc), errors='ignore')
                data = fp.read()
                voca_list = set([])
                ...
                for sentence in sentences:
                    #check such that the sentences does not belong to email format
                ...
                if len(voca_list)==0:
                    continue
                line = ",".join(voca_list)
                with open(os.path.join(process_dir, str(file_counter)), "w") as myfile:
                    myfile.write(line)
        ...
        print("The number of keywords %i" % len(inverted_dict)) # 29238
        print("The number of files %i" % (file_counter-1)) # 517401
        ...
        for keyword in inverted_dict:
            ...
            w_id_amount += len(inverted_dict[keyword])
        print("The number of keyword-file %i" % w_id_amount) # 37209489
