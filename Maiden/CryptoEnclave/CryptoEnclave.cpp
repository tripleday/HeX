...
void ecall_init(unsigned char *keyF, size_t len){ 
    ...
    uint64_t vector_size = 360000;//hold up 22 mil k,v // about 55MB
...
void ecall_printMem(){
    printf("%d", (sizeof(std::string)+sizeof(int))*ST.size()+(sizeof(std::string))*d.size()+(sizeof(std::string)+sizeof(int))*M_c.size());
}
